...
Item {
  ...
  function listScript() {
    if (root.sizeMode)
      return "current=$(omarchy-cursor-size-current 2>/dev/null); { omarchy-cursor-size-list 2>/dev/null | grep -qx \"$current\" && printf '%s\\t%s\\t%s\\n' 'Custom size' 'custom' \"$current\" || printf '%s\\t%s\\t%s\\n' 'Custom size' 'custom' \"$current\" '1'; omarchy-cursor-size-list 2>/dev/null | while read -r s; do [[ -z $s ]] && continue; printf '%s\\t%s\\t%s\\n' \"$s\" \"$s\" \"$current\"; done; }"
    return "current=$(omarchy-cursor-current 2>/dev/null); omarchy-cursor-list 2>/dev/null | while read -r c; do [[ -z $c ]] && continue; printf '%s\\t%s\\t%s\\n' \"$c\" \"$c\" \"$current\"; done"
  }
  ...
  function dismiss() {
    ...
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "omarchy.cursor-style")
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "omarchy-cursor-style"
